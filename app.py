...
def train_model():
    # Cargamos el dataset
    try:
        # Intentamos leer el csv. Si falla por encoding, probamos otro.
        df = pd.read_csv('heart_disease_uci.csv') 
    except:
        st.error("Error cargando 'heart_disease_uci.csv'. Verifica que el archivo esté en el repositorio.")
        st.stop()
    
    # --- LIMPIEZA ROBUSTA DE DATOS (CORRECCIÓN DEL ERROR) ---
    # 1. Reemplazamos '?' u otros caracteres raros por NaN (Vacío)
    df.replace('?', np.nan, inplace=True)
    
    # 2. Eliminamos columnas que no sirven para predecir (ID, Dataset)
    # Esto evita que el modelo falle por leer texto como "Cleveland"
    df = df.drop(columns=['id', 'dataset'], errors='ignore')
    
    # 3. Forzamos a que TODO sea numérico. Si algo no es número, se vuelve NaN
    for col in df.columns:
        df[col] = pd.to_numeric(df[col], errors='coerce')
        
    # 4. Eliminamos filas con valores vacíos
    df = df.dropna()
    # ---------------------------------------------------------
    
    # Definir X e y
    # Buscamos la columna objetivo (puede llamarse 'num' o 'target')
    target_col = 'num' if 'num' in df.columns else 'target'
    
    X = df.drop(columns=[target_col], errors='ignore')
    y = df[target_col].apply(lambda x: 1 if x > 0 else 0) # Convertir a binario (0 o 1)
    ...
    try:
        ...
        st.error(f"Error en el escalado de datos: {e}")
        st.stop()
...
def user_input_features():
    # Variables numéricas con valores por defecto seguros
...
if st.button('EJECUTAR DIAGNÓSTICO IA'):
    # Asegurar que el input tenga el mismo orden de columnas que el entrenamiento
    # Esto evita errores si las columnas están desordenadas
    ...
    if probability > 0.70:
        ...
        st.write("El modelo identifica patrones consistentes con enfermedad cardíaca.")
